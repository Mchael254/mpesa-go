...
export class ProductService {
  ...
  getListOfProduct(type = 'O') {
    return this.api.GET(`products?page=0&size=22&class_type=${type}`).pipe(
      map((_prod: any) => {
        return _prod['content'];
      })
      // catchError((_err) =>  {})
  ...
  getListOfProductTermByProductCode(
    ...
    return this.api
      .GET(
        `ord-prem-terms/product-option?product_code=${prodCode}&product_option_code=${prodPopCode}&client_age=${age}`
      )
      .pipe(
        map((_prod_term: any) => {
          return _prod_term;
