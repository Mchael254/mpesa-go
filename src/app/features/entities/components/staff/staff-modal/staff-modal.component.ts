...
export class StaffModalComponent implements OnInit, OnDestroy{
  @ViewChild('cancelUserSelection', {read: ElementRef}) cancelSupervisorSelect: ElementRef;
  ...
  lazyLoadedUsers: Pagination<StaffDto> = <Pagination<StaffDto>>{};
...
// The flag that indicates if the modal is open or not
  ...
  getIndividualUsers(pageIndex: number,
                     ...
                     order: string = 'desc') {
    return this.staffService.getStaff(pageIndex, this.staffSize, null, sortList, order, null)
      .pipe(untilDestroyed(this));
  }
  ...
  searchUsers(name:string){
    if(this.isLazyLoaded){
      ...
      for (let i = this.staffList.length - 1; i >= 0; i--) {
        if (!this.staffList[i]?.name.toLowerCase().includes(name.toLowerCase())) {
