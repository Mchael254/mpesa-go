...
export class NewEntityComponent implements OnInit {
          ...
          for (const key of Object.keys(this.entityRegistrationForm.controls)) {
            ...
            if (field.visibleStatus === 'Y') {
              if (key === field.frontedId && field.mandatoryStatus === 'Y') {
                this.entityRegistrationForm.controls[key].addValidators(
                  Validators.required
                );
                this.entityRegistrationForm.controls[
                  key
   ...
   */
  get f() {
    return this.entityRegistrationForm.controls;
  }
  ...
  onAssignRole(role) {
    ...
    this.entityRegistrationForm.controls['assign_role'].setValue(
      this.selectedRole?.partyTypeName
    );
  ...
  onSubmit() {
    ...
    this.entityService
      ...
      .subscribe({
        ...
        error: (err) => {
          this.findExistingEntity(
            'identityNumber',
            saveEntity.identityNumber,
            saveEntity
          );
  ...
  findExistingEntity(columnName: string, columnValue: string, saveEntity) {
    ...
    this.entityService
      .searchEntities(page, size, columnName, columnValue)
      .subscribe({
        next: (res: Pagination<EntityDto>) => {
          if (res.content.length === 0) {
            this.findExistingEntity(
              'pinNumber',
              saveEntity.pinNumber,
              saveEntity
            );
        ...
        error: (err) => {
          const errorMessage =
            err?.error?.message ?? err.message + `: ${err?.error?.errors[0]}`;
  ...
  redirectToViewExistingEntity(): void {
    this.router
      .navigate([`/home/entity/view/${this.foundEntity.id}`])
      .then((r) => true);
  }
  ...
  uploadImage(entityId: number) {
    this.entityService
      .uploadProfileImage(entityId, this.selectedFile)
      .subscribe((res) => {
        log.info(res);
