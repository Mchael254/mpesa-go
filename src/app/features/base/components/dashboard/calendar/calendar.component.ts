...
export class CalendarComponent implements OnInit {
  ...
  public daysOfTheWeek = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
  ...
  ngOnInit(): void {
    const todayDateString = this.createDateString(
      this.selectedDate,
      this.currMonth,
      this.currYear
    );
  ...
  createDateString(selectedDate, currMonth, currYear): string {
    ...
    return `${selectedDate < 10 ? '0' + selectedDate : selectedDate}/${
      adjustedMonth < 10 ? '0' + adjustedMonth : adjustedMonth
    }/${currYear}`;
  }
  ...
  selectDate(day: CalendarDay): void {
    ...
    const selectedDate: CalendarDay = this.daysToDisplay.filter(
      (x) => x.year === day.year && x.month === day.month && x.date === day.date
    )[0];

    const index = this.daysToDisplay.indexOf(selectedDate);

    this.daysToDisplay[index].isSelected =
      !this.daysToDisplay[index].isSelected;
  ...
  fetchFilteredEvents(): void {
    ...
    const dateRangeForEvents =
      this.selectedRange === DURATION.DAY
        ? 1
        : this.selectedRange === DURATION.WEEK
        ? 7
        : lastDayOfMonthIndex;

    this.calendarView =
      this.selectedRange === DURATION.MONTH
        ? DURATION.MONTH
        : this.selectedRange === DURATION.WEEK
        ? DURATION.WEEK
        : DURATION.DAY;
    ...
    const dayToSetActive = this.daysToDisplay.filter(
      (x) =>
        x.year === this.currYear &&
        x.month === this.currMonth &&
    ...
    if (
      ...
      for (let i = 1; i < 7; i++) {
        // todo: use start_index & end_index; remove constants
        this.daysToDisplay[index + i].isSelected =
          formValues.dateRange === DURATION.WEEK;
      }
    }
  ...
  filterEvents(dateString: string): CalendarEventReqDTO[] {
    return this.eventsData.filter((event) => {
      return (
        moment(new Date(event.startDate)).format('DD/MM/YYYY') === dateString
      );
    });
  }
  ...
  activateCreateEvent(calendarView): void {
    ...
    if (today > this.selectedDate) {
      this.globalMessagingService.displayInfoMessage(
        'Info',
        'You cannot create an event for previous date'
      );
      return;
    }
  ...
  sliceTime(start) {
    ...
    log.info(`selected date >>> `, start, eventDate.toISOString(), !!eventDate);

    if (eventDate) {
      this.newEventDate = eventDate.toISOString();
    } else {
      this.newEventDate = start;
    }
