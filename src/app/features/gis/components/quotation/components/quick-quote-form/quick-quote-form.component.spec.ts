...
describe('QuickQuoteFormComponent', () => {
  ...
  it('should toggle to a new client', fakeAsync(() => {
    // Create a spy for resetClientData method inside the test
    jest.spyOn(component, 'resetClientData').mockImplementation(() => { });

    // Set initial state
    component.new = true;
    component.clientName = 'John Doe'; // Set some initial values

    // Call the method to toggle to a new client
    component.toggleNewClient();

    // Use fakeAsync and tick to handle asynchronous operations if any
    tick();

    // Ensure that change detection has been triggered
    fixture.detectChanges();

    // Use fixture.whenStable for any asynchronous operations
    fixture.whenStable().then(() => {
      // Verify that the properties are reset and new is set to false
      expect(component.new).toBeFalsy();

      // Verify that the clientName is now an empty string
      expect(component.clientName).toEqual('');

      // Ensure that the resetClientData method is called
      expect(component.resetClientData).toHaveBeenCalled();
    });
  }));
  ...
  // it('should load and set the client list', () => {
  //   jest.spyOn(clientService, 'getClients').mockReturnValue(of(mockPagination));

  // // Call the method to load the client list
  // component.loadAllClients();

  // // Ensure that the clientService.getClients method is called
  // expect(clientService.getClients).toHaveBeenCalled();

  // // Ensure that the clientList and clientData are set correctly based on the mocked data
  // expect(component.clientList).toEqual(mockPagination.content);
  // expect(component.clientData).toEqual(mockPagination.content.map(client => client.clientTypeName));
  ...
  it('should set selectedProductCode, call methods, and log the correct message', () => {
    ...
    // Spy on console.log to check if it is called with the correct message
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Selected Product Code:', component.selectedProductCode);
  });
  ...
  it('should load all currencies and set selected currency', async () => {
    ...
    // Set a mock currency code for testing
    ...
    expect(component.selectedCurrency).toEqual('US Dollar');
  ...
  it('should load form fields based on selected product code', () => {
    ...
    jest.spyOn(quotationService, 'getFormFields').mockReturnValue(of(mockFormContent));
    ...
    expect(component.formContent).toEqual(mockFormContent);
    expect(component.formData).toEqual(mockFormContent.fields);

    // Check if form controls are added
    mockFormContent.fields.forEach((field) => {
  ...
  it('should load subclass section cover types', () => {
    const mockSubclassSectionCoverList = [
      { subClassCode: '123', isMandatory: 'Y', /* add more properties as needed */ },
      { subClassCode: '123', isMandatory: 'N', /* add more properties as needed */ },
    ...
    expect(component.mandatorySections).toEqual(mockSubclassSectionCoverList.filter(section => section.subClassCode == '123' && section.isMandatory == 'Y')as any);
    expect(component.selectedSectionList).toEqual(mockSubclassSectionCoverList.filter(section => section.subClassCode == '123' && section.isMandatory == 'Y')[0]);
