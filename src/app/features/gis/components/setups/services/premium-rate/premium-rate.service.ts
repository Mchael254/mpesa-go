...
import { NewPremiums, Premiums } from '../../data/gisDTO';
...
export class PremiumRateService {
  ...
  getCoverTypePremiums(subClassCode: number, binderCode: number, coverTypeCode: number): Observable<NewPremiums[]> {
    return this.api.GET<NewPremiums[]>(`api/v1/premium-rates/cover-type?binderCode=${binderCode}&subclassCode=${subClassCode}&coverTypeCode=${coverTypeCode}`, API_CONFIG.GIS_SETUPS_BASE_URL).pipe(
      retry(1),
      catchError(this.errorHandl)
    )
  }
